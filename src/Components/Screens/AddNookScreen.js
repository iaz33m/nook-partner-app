...
class AddNookScreen extends React.Component {
  ...
  getArea = () => {
    const {getArea, user: { access_token: token } } = this.props;
 ...
 var room = {capacity,noOfBeds,price_per_bed};
    this.setState(prevState => ({
      rooms: [ {"capacity": capacity, "noOfBeds":noOfBeds, "price_per_bed":price_per_bed}, ...prevState.rooms],
      capacity:"",
      isDialogVisible: false,
    }))
}
  ...
  renderRoomPopup = () => {
    const { isSchedule, isDialogVisible,submitting, capacity, noOfBeds, price_per_bed } = this.state;

    if (isSchedule) {
      return (
        <PopupDialog
          width={0.9} height={0.70}
          ...
          <View style={{ flex: 1, padding: 25, }}>
            ...
            <Text style={{justifyContent: 'center', fontWeight: 'bold'}}>Rooms</Text>
        ...
        <View >
          <Item picker style={styles.pickerStyle}>
            <Picker
              ...
              {(area.data).map(k => <Picker.Item key={k.id} label={k.area} value={k.sub_area} />)}
            </Picker>
          </Item>
        </View>
      </View>
        ...
        <View >
          <Item picker style={styles.pickerStyle}>
            <Picker
              ...
              {(mainArea).map(k => <Picker.Item  label={k.name} value={k.locations} />)}
            </Picker>
          </Item>
        </View>
      </View>
        ...
        <View >
          <Item picker style={styles.pickerStyle}>
            <Picker
              ...
              {(areaLocation).map(k => <Picker.Item  label={k.name} value={k} />)}
            </Picker>
          </Item>
        </View>
      </View>
            ...
            <View style={[styles.container, {
              ...
              <View style={styles.child}>
                <View style={[styles.childItem, {
                  ...
                  <View style={{ flex: 1, alignItems: 'flex-end' }}>
                    <AirbnbRating showRating={false} size={20} defaultRating={4} onFinishRating={review => this.setState({ review })}/>
                  </View>
                </View>
                <View style={styles.checkbox}>
                  <View style={styles.checkboxItem}>
                    <Text>Fernished</Text>
                    <CheckBox checked={this.state.Fernished} onPress={() => this.setState({Fernished: !this.state.Fernished})} />
                  </View>
                  <View style={styles.checkboxItem}>
                    <Text>AC</Text>
                    <CheckBox checked={this.state.AC} onPress={() => this.setState({AC: !this.state.AC})} />
                  </View>
                  <View style={styles.checkboxItem}>
                    <Text>TV</Text>
                    <CheckBox checked={this.state.TV} onPress={() => this.setState({TV: !this.state.TV})} />
                  </View>
                </View>
                <View style={[styles.checkbox, { paddingTop: 0 }]}>
                  <View style={styles.checkboxItem}>
                    <Text>Wifi</Text>
                    <CheckBox checked={this.state.Wifi} onPress={() => this.setState({Wifi: !this.state.Wifi})} />
                  </View>
                  <View style={styles.checkboxItem}>
                    <Text>CCTV</Text>
                    <CheckBox checked={this.state.CCTV} onPress={() => this.setState({CCTV: !this.state.CCTV})} />
                  </View>
                  <View style={styles.checkboxItem}>
                    <Text>UPS</Text>
                    <CheckBox checked={this.state.UPS} onPress={() => this.setState({UPS: !this.state.UPS})} />
                  </View>
                </View>
              </View>
            </View>
            ...
            <InputField
              ...
              <View style={styles.container}>
                <View style={[styles.child, { borderRadius: 30, flex: 1, justifyContent: 'space-between', flexDirection: 'row', alignItems: 'center', paddingStart: 15, paddingEnd: 15 }]}>
                    <Text style={{ margin: 15, fontSize: 16, fontWeight: 'bold' }}>Room</Text>
                    <TouchableOpacity
